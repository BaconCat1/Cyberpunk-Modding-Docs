...
Steam does not let you choose a different executable on macOS, so the workaround is to temporarily replace the top-level launcher with the script. On macOS, use `$HOME` in these commands, not `"~"`, because `~` will not expand inside quotes.

Directly launching the wrapped binary from Terminal will usually fail with `SteamAPI_Init()` because it is not being started by Steam. After completing these steps, test by launching the game from the Steam client, not by running the app bundle executable manually in Terminal.
